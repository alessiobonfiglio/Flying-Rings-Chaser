...
class Camera extends GameObject {
	...
	async tilt() {
		...
		const tiltAnimation = async () => {
			...
			const noiseResolution = 1.5;
			const noiseFactor = 0.1;
			const curNoiseX = angle => noise.simplex2(Math.cos(angle) * noiseResolution, 2 * Math.sin(angle) * noiseResolution) - 0.5;
			const curNoiseY = angle => noise.simplex2(2 * Math.cos(angle) * noiseResolution, Math.sin(angle) * noiseResolution) - 0.5;

			await Promise.all([

				Animations.lerp(angle => this.position = MathUtils.sum(this.#computeCurrentPosition(), MathUtils.mul(noiseFactor, [curNoiseX(angle), curNoiseY(angle), 0])),
